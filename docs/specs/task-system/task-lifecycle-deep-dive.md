...
> **Context:** The universal task table exists (see [overview.md](./overview.md)). This document answers: _What actually happens when something triggers a task?_ How does a document annotation, an inbound message, or a deal stage change become a task with a builder, modifier, and signer? What does the task builder UX look like? How do automation rules fire?
