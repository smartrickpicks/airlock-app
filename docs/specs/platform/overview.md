...
This spec defines the three-layer architecture, the monetization model, how feature flags become marketplace entitlements, and the tiered build strategy (dogfood first, then multi-tenant).

> **Vocabulary note:** This spec uses "Tier 1/2/3" for project milestones (dogfood → multi-tenant → platform). These are NOT Airlock Chambers. "Channel" appears only when referencing external Slack channels — Airlock's canonical type is **ConversationSpace**. See `CLAUDE.md` for vocabulary rules.
...
├── journeys.json        ← lifecycle chamber definitions (Discover > Build > Review > Ship)
...
### Tier 2: Multi-tenant (Other orgs)

**Goal:** Extract your setup into a configurable template for new customers.

| What                  | How                                                         |
| --------------------- | ----------------------------------------------------------- |
| **Onboarding wizard** | Steps 0–6 from research doc (see `onboarding-flow.md`)      |
...
| Channel/Room (external)                 | **Vault** (workflow) or **ConversationSpace** (comms) | Vaults are workflow instances, NOT chat rooms. Slack channels map to ConversationSpaces.                                       |
...
| D2  | Marketplace lives in Admin Overlay       | **LOCKED: (a) → then (b)** | Start as Admin > Connectors > Marketplace tab. Tier 2: expose user-facing config for non-admins (Discord integrations model — admin enables, users configure their own).                                                           |
...
**Deferred to Tier 2:** Department-based view filtering. Tier 1 uses role-based filtering only.

---

## Database Extensions Required (Tier 2)
...
| `onboarding/overview.md`            | Extends with Step 0-6 provisioning flow from research doc.                                                |
...
### Now (Tier 1 — Dogfood, 6-8 weeks)

See `docs/specs/platform/connector-roadmap.md` for detailed Tier 1 connector plan.

**Week 1-2: Context server + canonical model**

1. Stand up context MCP server (orgs, users, workspaces, roles, connector configs)
2. Define canonical JSON schemas: Task, Event, MessageThread, ConversationSpace, Document
