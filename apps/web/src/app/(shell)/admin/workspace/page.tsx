  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-2xl space-y-6">
        ...
        <section className="rounded-lg border border-surface-border bg-surface-raised">
          ...
          <div className="space-y-4 p-4">
            ...
            <div className="flex items-center justify-between pt-1">
              <span className="text-xs text-text-muted font-mono">{MOCK_WORKSPACE.id}</span>
              <button
                onClick={handleSave}
        ...
        <section className="rounded-lg border border-surface-border bg-surface-raised">
          <div className="border-b border-surface-border px-4 py-3">
            <h2 className="text-sm font-semibold text-text-primary">Plan & usage</h2>
          </div>
          <div className="divide-y divide-surface-border">
            {([
              ["Plan", <span key="plan" className={`rounded-full px-2 py-0.5 text-xs font-medium capitalize ${PLAN_BADGE[MOCK_WORKSPACE.plan]}`}>{MOCK_WORKSPACE.plan}</span>],
              ["Members", <span key="members" className="text-sm text-text-primary">{MOCK_WORKSPACE.memberCount}</span>],
              ["Vaults", <span key="vaults" className="text-sm text-text-primary">{MOCK_WORKSPACE.vaultCount}</span>],
              ["Created", <span key="created" className="text-sm text-text-secondary">{new Date(MOCK_WORKSPACE.createdAt).toLocaleDateString()}</span>],
        ...
        <section className="rounded-lg border border-surface-border bg-surface-raised">
          <div className="border-b border-surface-border px-4 py-3">
            <h2 className="text-sm font-semibold text-text-primary">Invite link</h2>
          </div>
          <div className="p-4">
          ...
          <div className="flex items-center justify-between p-4">
            <div>
              ...
              <p className="mt-0.5 text-xs text-text-muted">
                Freezes all vaults. Members lose access. Irreversible without support.
              </p>
            </div>
