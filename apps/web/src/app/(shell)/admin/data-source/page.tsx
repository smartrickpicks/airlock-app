...
const STATUS_CONFIG = {
  connected: { label: "Connected", color: "bg-accent-success/15 text-accent-success", dot: "bg-accent-success" },
  pending: { label: "Pending", color: "bg-accent-warning/15 text-accent-warning", dot: "bg-accent-warning" },
  error: { label: "Error", color: "bg-accent-error/15 text-accent-error", dot: "bg-accent-error" },
  not_configured: { label: "Not configured", color: "bg-text-muted/15 text-text-muted", dot: "bg-text-muted" },
};
      ...
      <div className="max-w-2xl space-y-6">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary">
            Manage database connections and external data imports. Internal sources are managed automatically.
          </p>
        </div>
        ...
        <div className="divide-y divide-surface-border rounded-lg border border-surface-border bg-surface-raised">
          {MOCK_DATA_SOURCES.map((source) => {
            ...
            return (
              <div key={source.id} className="flex items-start justify-between p-4">
