      ...
      {role.isSystem && (
        <div className="flex items-center gap-2 rounded-md border border-surface-border bg-surface-sunken px-3 py-2">
          <span className="text-xs text-text-muted">
            <strong className="text-text-secondary">System role</strong> — cannot be deleted or renamed.
            Permissions are shown as a reference in the Permissions tab.
          </span>
        </div>
      )}
    </div>
    ...
    <div className="divide-y divide-surface-border rounded-lg border border-surface-border">
      {matched.map((m) => (
        <div key={m.id} className="flex items-center gap-3 px-3 py-2.5">
          ...
          <span className="text-xs text-text-muted capitalize">{m.orgRole}</span>
        </div>
      ))}
    </div>
  );
...
function CreateRoleForm({
  ...
  return (
    <div className="flex flex-col gap-5 p-6">
      <div>
        <h3 className="text-base font-semibold text-text-primary">Create role</h3>
        <p className="mt-0.5 text-xs text-text-secondary">
          New roles start with no permissions. Add them in the Permissions tab after creating.
        </p>
      </div>
...
export default function AdminRolesPage() {
  ...
  const [selectedId, setSelectedId] = useState<string | "new" | null>("architect");
    ...
    <div className="flex h-full overflow-hidden">
      ...
      <aside className="flex w-56 flex-shrink-0 flex-col border-r border-surface-border bg-surface-raised overflow-y-auto">
        <div className="border-b border-surface-border px-3 py-3">
          <button
            onClick={() => { setSelectedId("new"); setActiveTab("display"); }}
        ...
        <div className="flex-1 px-1 py-2 space-y-0.5">
          ...
          {roles.filter((r) => r.isSystem).map((role) => (
            <button
              key={role.id}
              onClick={() => { setSelectedId(role.id); setActiveTab("display"); }}
            ...
            >
              ...
              {role.memberCount > 0 && (
                <span className="text-[10px] text-text-muted">{role.memberCount}</span>
              )}
            </button>
          ))}
        ...
        {isCreating ? (
          ...
          <div className="flex h-full flex-col">
            ...
            <div className="flex flex-shrink-0 gap-0 border-b border-surface-border px-5">
              {(["display", "permissions", "members"] as RoleTab[]).map((tab) => (
                <button
                  key={tab}
