...
export default function CreateModePanel({ artifactType, onApprove }: CreateModePanelProps) {
  const { session, startSession, submitAnswer, approvePlan } = useBrainstormStore();
  ...
  const artifactLabel = artifactType.charAt(0).toUpperCase() + artifactType.slice(1);
      ...
      <div className="flex h-10 flex-shrink-0 items-center border-b border-surface-border bg-surface-raised px-4">
        ...
        {session.phase === "qa" && session.history.length > 0 && (
          <span className="ml-auto text-[10px] text-text-muted">
            ...
            {session.history.length + (session.currentQuestion ? 1 : 0)} questions answered
          </span>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5">
        ...
        {session.phase === "qa" && session.currentQuestion && (
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-text-primary">{session.currentQuestion.question}</p>
          </div>
        )}
        ...
        {session.phase === "plan" && session.planOutput && (
          <div className="rounded-lg border border-surface-border bg-surface-raised p-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-text-muted">— Plan ready for review</span>
            </div>
